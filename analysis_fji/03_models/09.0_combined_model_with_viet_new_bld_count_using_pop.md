...
# Load Fiji complete interpolated new
...
df_fji_complete_int_new2 = get_training_dataset_complete_interpolated_new_bld_count_using_pop()
df_fji_complete_int_new2 = df_fji_complete_int_new2[df_fji_complete_int_new2.typhoon_name != 'ANA'] # Drop ANA
df_fji_complete_int_new2 = df_fji_complete_int_new2.rename({
    "mean_altitude": "mean_elev",
    "total_buildings": "total_houses",
    "perc_dmg_grid": "percent_houses_damaged"
}, axis=1)
df_fji_complete_int_new2 = df_fji_complete_int_new2[features + ['typhoon_name', 'grid_point_id', 'Centroid', 'percent_houses_damaged']]
...
df_combined_with_viet_new2 = get_combined_dataset_interpolated_with_viet_new_bld_count_using_pop()
df_fji_with_viet_new2 = df_combined_with_viet_new2[df_combined_with_viet_new2.country == 'fji']
```
...
```
    ...
    /Users/federico/Library/CloudStorage/OneDrive-Personal/Documentos/ISI_Project/FIJI clean/analysis_fji/03_models/utils.py:564: RuntimeWarning: Mean of empty slice
      test_rmse_bin = np.nanmean(np.array(rmse_bin_fji)[:,i])
    /Users/federico/Library/CloudStorage/OneDrive-Personal/Documentos/ISI_Project/FIJI clean/analysis_fji/03_models/utils.py:567: RuntimeWarning: Mean of empty slice
      test_avg_bin = np.nanmean(np.array(avg_error_bin_fji)[:,i])



```python
# Fji + Phl + Viet NEW BLD COUNT USING BLD
y_test_typhoon_with_viet_new2, y_pred_typhoon_with_viet_new2, rmse_strat_with_viet_new2, avg_error_strat_with_viet_new2 = xgb_model_combined_data_with_viet_LOOCV(
    df_combined=df_combined_with_viet_new2,
    df_fji=df_fji_with_viet_new2,
    bins=bins_def_fji,
    fji_weight=3,
    features=features
)
...
# New approach PHL+FJI+VIET new bld count USING POP DATA
fiji_new2 = fiji.copy()
fiji_new2['actual_perc_dmg'] = fiji_new2.apply(calculate_actual_perc_dmg, df_fji=df_fji_complete_int_new2,
                                             y_pred_typhoon=y_pred_typhoon_with_viet_new2, i=1, typhoon=typhoon, axis=1)
fiji_new2['pred_perc_dmg'] = fiji_new2.apply(calculate_pred_perc_dmg, df_fji=df_fji_complete_int_new2,
                                           y_pred_typhoon=y_pred_typhoon_with_viet_new2, i=1, typhoon=typhoon, axis=1)
# Modify values
fiji_new2['actual_perc_dmg'] = fiji_new2['actual_perc_dmg'].apply(lambda x: 0 if x < 0 else (100 if x > 100 else x))
fiji_new2['pred_perc_dmg'] = fiji_new2['pred_perc_dmg'].apply(lambda x: 0 if x < 0 else (100 if x > 100 else x))
fiji_new2['prediction_error'] = fiji_new2['actual_perc_dmg'] - fiji_new2['pred_perc_dmg'] # in percentual points
```
...
```
    ...
    2024-03-06 13:49:41,809 - climada.hazard.tc_tracks - WARNING - The cached IBTrACS data set dates from 2023-06-07 23:07:38 (older than 180 days). Very likely, a more recent version is available. Consider manually removing the file /Users/federico/climada/data/IBTrACS.ALL.v04r00.nc and re-running this function, which will download the most recent version of the IBTrACS data set from the official URL.
...
positive_error = fiji_new[fiji_new['prediction_error'] > 0]
negative_error = fiji_new[fiji_new['prediction_error'] < 0]
positive_error_int = fiji_new2[fiji_new2['prediction_error'] > 0]
negative_error_int = fiji_new2[fiji_new2['prediction_error'] < 0]

# Plotting the maps
fiji_plot_1 = fiji_new.plot(column='actual_perc_dmg', cmap=cmap, linewidth=0.2, ax=ax[0], edgecolor='0.3', legend=True)
fiji_plot_2 = fiji_new.plot(column='pred_perc_dmg', cmap=cmap, linewidth=0.2, ax=ax[1], edgecolor='0.3', legend=True)
...
fiji_plot_1_int = fiji_new2.plot(column='actual_perc_dmg', cmap=cmap, linewidth=0.2, ax=ax[3], edgecolor='0.3', legend=True)
fiji_plot_2_int = fiji_new2.plot(column='pred_perc_dmg', cmap=cmap, linewidth=0.2, ax=ax[4], edgecolor='0.3', legend=True)
...
ax[1].set_title('Typhoon {} (PHL+FJI+VIET model NEW BLD COUNT (using urban-rural data)) \n \nPredicted Damage by municipality'.format(typhoon))
ax[2].set_title('Prediction Error \n $actual_{dmg} - predicted_{dmg}$ \n(in percentage points)', y=0.95)
ax[4].set_title('Typhoon {} (PHL+FJI+VIET model NEW BLD COUNT (using census data))'.format(typhoon))
ax[0].axis('off')
ax[1].axis('off')
ax[2].axis('off')
ax[3].axis('off')
...
![png](09.0_combined_model_with_viet_new_bld_count_using_pop_files/09.0_combined_model_with_viet_new_bld_count_using_pop_14_0.png)
...
# Plotting the maps
fiji_plot_1_int = fiji_new2.plot(column='actual_perc_dmg', cmap=cmap, linewidth=0.2, ax=ax[0], edgecolor='0.3', legend=True)
fiji_plot_2_int = fiji_new2.plot(column='pred_perc_dmg', cmap=cmap, linewidth=0.2, ax=ax[1], edgecolor='0.3', legend=True)
fiji_plot_3_blue_int = positive_error_int.plot(column='prediction_error', cmap=cmap_blue, linewidth=0.2, ax=ax[2], edgecolor='0.3', vmin=0, legend=True)
fiji_plot_3_red_int = negative_error_int.plot(column='prediction_error', cmap=cmap_red, linewidth=0.2, ax=ax[2], edgecolor='0.3', vmax=0, legend=True)
...
ax[0].set_title('Actual % of  Damage by municipality')
ax[1].set_title('Typhoon {} (PHL+FJI+VIET model), XGBoost with LOOCV \n \nPredicted % of Damage by municipality'.format(typhoon))
ax[2].set_title('Prediction Error \n $actual_{dmg} - predicted_{dmg}$ \n(in percentage points)', y=0.95)
...
![png](09.0_combined_model_with_viet_new_bld_count_using_pop_files/09.0_combined_model_with_viet_new_bld_count_using_pop_15_0.png)
...
for typhoon in typhoons:
    ...
    actual_dmg_new2 = fiji_new2['actual_perc_dmg']
    pred_dmg_viet = fiji_old['pred_perc_dmg']
    pred_dmg_viet_new = fiji_new['pred_perc_dmg']
    pred_dmg_viet_new2 = fiji_new2['pred_perc_dmg']
...
for i in range(len(muns)):
    ...
    #RMSE with VIET and new bld count using pop data
...
plt.plot(rmse_mun_fji_viet_new, 'o-', label='PHL+FJI+VIET model NEW bld count (urban-rural)', alpha=0.6)
plt.plot(rmse_mun_fji_viet_new2, 'o-', label='PHL+FJI+VIET model NEW bld count (ArcGIS HUB)', alpha=0.6)
...
![png](09.0_combined_model_with_viet_new_bld_count_using_pop_files/09.0_combined_model_with_viet_new_bld_count_using_pop_19_0.png)
...
error_new2 = fiji_new2['prediction_error']
muns = fiji_old['NAME_2']
```
...
plt.plot(error_new, 'o-', label='PHL+FJI+VIET model, \nUrban-Rural approach', alpha=0.4)
plt.plot(error_new2, 'o-', label='PHL+FJI+VIET model, \nnew bld count (ArcGIS HUB)', alpha=0.4)
...
![png](09.0_combined_model_with_viet_new_bld_count_using_pop_files/09.0_combined_model_with_viet_new_bld_count_using_pop_23_0.png)
