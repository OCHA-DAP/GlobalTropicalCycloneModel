...
from utils import get_combined_dataset_interpolated, get_combined_dataset_interpolated_with_viet, get_training_dataset_complete_interpolated, get_municipality_grids, xgb_model_combined_data_LOOCV, xgb_model_combined_data_with_viet_LOOCV
```
...
df_combined_no_viet = get_combined_dataset_interpolated()
df_combined_with_viet = get_combined_dataset_interpolated_with_viet()
df_fji_no_viet = df_combined_no_viet[df_combined_no_viet.country == 'fji']
...
```python
# Stratification
dmg = np.array(df_fji_no_viet.percent_houses_damaged.to_list())
...
y_test_typhoon_fji, y_pred_typhoon_fji, rmse_strat_fji, avg_error_strat_fji= xgb_model_combined_data_LOOCV(
    df_combined=df_fji_no_viet,
    df_fji=df_fji_no_viet,
    bins=bins_def_fji,
    fji_weight=1,
    features=features
)
...
```python
# Fji + Phl
y_test_typhoon_no_viet, y_pred_typhoon_no_viet, rmse_strat_no_viet, avg_error_strat_no_viet = xgb_model_combined_data_LOOCV(
    df_combined=df_combined_no_viet,
    df_fji=df_fji_no_viet,
    bins=bins_def_fji,
    fji_weight=3,
    features=features
...
#ax.plot(range(num_bins_fji), rmse_strat_fji, 'rs', label='FJI')
ax.plot(range(num_bins_fji), rmse_strat_no_viet, 'bs', label='PHL + FJI --> Weight Ratio= 3:1')
ax.plot(range(num_bins_fji), rmse_strat_with_viet, 'gs', label='PHL + FJI + VIET --> Weight Ratio= 3:1:1')
...
![png](07.0_combined_model_with_viet_files/07.0_combined_model_with_viet_11_0.png)
...
```python
# Fiji typhoons
fji_typhoons = df_fji_no_viet.typhoon_name.unique()
...
# New approach PHL+FJI+VIET
fiji_new = fiji.copy()
fiji_new['actual_perc_dmg'] = fiji_new.apply(calculate_actual_perc_dmg, df_fji=df_fji_complete_int,
                                             y_pred_typhoon=y_pred_typhoon_with_viet, i=1, typhoon=typhoon, axis=1)
fiji_new['pred_perc_dmg'] = fiji_new.apply(calculate_pred_perc_dmg, df_fji=df_fji_complete_int,
                                           y_pred_typhoon=y_pred_typhoon_with_viet, i=1, typhoon=typhoon, axis=1)
# Modify values
...
```
    ...
    2024-02-06 15:45:36,271 - climada.hazard.tc_tracks - WARNING - The cached IBTrACS data set dates from 2023-06-07 23:07:38 (older than 180 days). Very likely, a more recent version is available. Consider manually removing the file /Users/federico/climada/data/IBTrACS.ALL.v04r00.nc and re-running this function, which will download the most recent version of the IBTrACS data set from the official URL.
...
![png](07.0_combined_model_with_viet_files/07.0_combined_model_with_viet_16_0.png)
...
for typhoon in typhoons:
    ...
    # Just FJI approach
    fiji_clas = fiji.copy()
    fiji_clas['actual_perc_dmg'] = fiji_clas.apply(calculate_actual_perc_dmg, df_fji=df_fji_complete_int,
                                                y_pred_typhoon=y_pred_typhoon_fji, i=1, typhoon=typhoon, axis=1)
    fiji_clas['pred_perc_dmg'] = fiji_clas.apply(calculate_pred_perc_dmg, df_fji=df_fji_complete_int,
                                            y_pred_typhoon=y_pred_typhoon_fji, i=1, typhoon=typhoon, axis=1)

    # Modify values
    fiji_clas['actual_perc_dmg'] = fiji_clas['actual_perc_dmg'].apply(lambda x: 0 if x < 0 else (100 if x > 100 else x))
    fiji_clas['pred_perc_dmg'] = fiji_clas['pred_perc_dmg'].apply(lambda x: 0 if x < 0 else (100 if x > 100 else x))
    # Now calculate prediciton error
    fiji_clas['prediction_error'] = fiji_clas['actual_perc_dmg'] - fiji_clas['pred_perc_dmg'] # in percentual points
    ...
    pred_dmg_noviet = fiji_old['pred_perc_dmg']
    pred_dmg_viet = fiji_new['pred_perc_dmg']
    pred_dmg_fji = fiji_clas['pred_perc_dmg']
    ...
    for i in range(len(muns)):
        rmse_noviet.append(np.sqrt(mean_squared_error([actual_dmg[i]], [pred_dmg_noviet[i]])))
        rmse_viet.append(np.sqrt(mean_squared_error([actual_dmg[i]], [pred_dmg_viet[i]])))
        rmse_fji.append(np.sqrt(mean_squared_error([actual_dmg[i]], [pred_dmg_fji[i]])))

    rmse_noviet_tot.append(rmse_noviet)
    rmse_viet_tot.append(rmse_viet)
    rmse_fji_tot.append(rmse_fji)
```
...
for i in range(len(muns)):
    #RMSE no VIET
    test_rmse_mun = np.nanmean(np.array(rmse_noviet_tot)[:,i])
    rmse_mun_fji_noviet.append(test_rmse_mun)

    #RMSE with VIET
    test_rmse_mun_viet = np.nanmean(np.array(rmse_viet_tot)[:,i])
    rmse_mun_fji_viet.append(test_rmse_mun_viet)

    #RMSE just FJI
    test_rmse_mun_fji = np.nanmean(np.array(rmse_fji_tot)[:,i])
    rmse_mun_fji.append(test_rmse_mun_fji)
```


```python
plt.plot(rmse_mun_fji, 'o-', label='FJI model', alpha=0.8)
plt.plot(rmse_mun_fji_noviet, 'o-', label='PHL+FJI model', alpha=0.8)
...
![png](07.0_combined_model_with_viet_files/07.0_combined_model_with_viet_20_0.png)
...
```python
plt.plot(actual_dmg, 'o-',label='Real damage', alpha=1)
plt.plot(pred_dmg_noviet, 'o-', label='Pred damage: PHL+FJI model', alpha=0.4)
plt.plot(pred_dmg_viet, 'o-',label='Pred damage: PHL+FJI+VIET model', alpha=0.4)

plt.xticks(range(len(muns)), muns, rotation='vertical')  # Set x-axis ticks and labels
plt.tight_layout()  # Adjust layout to prevent overlapping
plt.title('Tropical Cyclone: {}'.format(typhoon))
plt.ylabel('% of damage')
...
![png](07.0_combined_model_with_viet_files/07.0_combined_model_with_viet_23_0.png)
