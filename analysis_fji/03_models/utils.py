...
def get_training_dataset_complete_smooth():
    input_dir = (
        Path(os.getenv("STORM_DATA_DIR")) / "analysis_fji/03_model_input_dataset"
    )
    filename = input_dir / "new_model_training_dataset_fji_complete_smooth_wind.csv"
    return pd.read_csv(filename)

def get_training_dataset_complete_interpolated():
    input_dir = (
        Path(os.getenv("STORM_DATA_DIR")) / "analysis_fji/03_model_input_dataset"
    )
    filename = input_dir / "new_model_training_dataset_fji_complete_interpolated_wind.csv"
    return pd.read_csv(filename)

def get_training_dataset_complete_interpolated_new_bld_count():
    input_dir = (
        Path(os.getenv("STORM_DATA_DIR")) / "analysis_fji/03_model_input_dataset"
    )
    filename = input_dir / "new_model_training_dataset_fji_complete_interpolated_wind_new_bld_count.csv"
    return pd.read_csv(filename)

def get_training_dataset_complete_interpolated_new_bld_count_using_pop():
    input_dir = (
        Path(os.getenv("STORM_DATA_DIR")) / "analysis_fji/03_model_input_dataset"
    )
    filename = input_dir / "new_model_training_dataset_fji_complete_interpolated_wind_new_bld_count_using_pop.csv"
    return pd.read_csv(filename)
...
def get_combined_dataset_smooth():
    # Load both datasets
    df_fji = get_training_dataset_complete_smooth()
    df_phl = get_training_dataset_phl()
    # Standardized features names
    df_fji = df_fji.rename(
        {'perc_dmg_grid':'percent_houses_damaged',
...
def get_combined_dataset_interpolated_with_viet():
    # Load datasets
    df_fji = get_training_dataset_complete_interpolated()
    df_phl = get_training_dataset_phl()
...
def get_combined_dataset_interpolated_with_viet_new_bld_count():
    # Load datasets
    df_fji = get_training_dataset_complete_interpolated_new_bld_count()
    df_phl = get_training_dataset_phl()
    df_viet = get_training_dataset_viet()
    # Standardized features names
    df_fji = df_fji.rename(
...
def get_combined_dataset_interpolated_with_viet_new_bld_count_using_pop(rwi_features=False):
    # Load datasets
    df_fji = get_training_dataset_complete_interpolated_new_bld_count_using_pop()
    df_phl = get_training_dataset_phl()
    df_viet = get_training_dataset_viet()
    # Standardized features names
    df_fji = df_fji.rename(
...
def get_combined_dataset_interpolated():
    # Load datasets
    df_fji = get_training_dataset_complete_interpolated()
    df_phl = get_training_dataset_phl()
    # Standardized features names
...
def xgb_model_combined_data_LOOCV(df_combined, df_fji, features, bins, fji_weight, phl_weight=1, viet_weight=1):
    # Dataframe Fiji
...
def xgb_model_combined_data_with_viet_LOOCV(df_combined, df_fji, features, bins, fji_weight, phl_weight=1, viet_weight=1):
    # Dataframe Fiji
    fji_typhoons = df_fji.typhoon_name.unique()
