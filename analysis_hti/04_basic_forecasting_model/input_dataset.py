...
from utils import get_stationary_data_hti
...
def create_windfield_dataset(thres=120, deg=3):
    ...
    # Load grids
    grids = gpd.read_file(output_dir / "hti_0.1_degree_grid_land_overlap.gpkg")
    ...
    thres_polygon = Polygon([(xmin, ymin), (xmin, ymax), (xmax, ymax), (xmax, ymin)])
    ...
    for i, intensity_sparse in enumerate(tc.intensity):
        ...
        intersects_region = tc_track_line.intersects(thres_polygon)
        ...
        df_to_add = pd.DataFrame(
            dict(
                ...
                in_region=[intersects_region] * npoints,
...
def trigger_hti(df_windfield):
    length = len(df_windfield[df_windfield.in_region == True]) # In the polygone
    if length == 0:
        return False
    else:
        return True
...
def create_rainfall_dataset(df_windfield):
    ...
    # Focus on the region
    region_forecast = df_windfield[df_windfield.in_region == True]

    # Group the DataFrame by the 'unique_id' column
    grouped = region_forecast.groupby('unique_id')
    ...
    event_metadata = pd.DataFrame({
        'event':region_forecast.unique_id.unique(),
...
def create_input_dataset(df_windfield, df_rainfall):
    ...
    # Stationaty data
    df_stationary = get_stationary_data_hti()
    ...
    variable_data = region_forecast.merge(df_rainfall, left_on=['unique_id','grid_point_id'], right_on=['event', 'id'])[
        ['grid_point_id',
        'event_id_ecmwf',
        'unique_id',
        'time_init',
        ...
        'track_distance',
        'rainfall_max_6h',
        'rainfall_max_24h']].sort_values('rainfall_max_6h', ascending=False)

    # Merge everything
    input_df = df_stationary.merge(variable_data, left_on='grid_point_id', right_on='grid_point_id')[
        ['grid_point_id',
        'IWI',
        'total_buildings',
        'with_coast',
